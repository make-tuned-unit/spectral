...
**Date:** 2026-04-28
**Source:** Production constellation/TACT at `/Users/cortex/memory/`
...
Measured via `tact_retrieval.py benchmark-compare` across 8 representative queries (auth decision, coffee preference, apollo strategy, stripe setup, alice anniversary, acme roadmap, cortex infrastructure, love nova scotia grant).
...
```python
PROJECT_RULES = [
    (re.compile(r"acme|ladle|bob.jones|recipe", re.I), "acme", "Acme"),
    (re.compile(r"apollo|polymarket|prediction|wager", re.I), "apollo", "Apollo"),
    ...
]
```
...
```python
WING_RULES = [
    (r"alice|coffee|anniversary|colou?r|favourit|favorit|sons|noah|leo|carol.doe", "alice"),
    ...
]
```
...
**Why it helps:** Avoids redundant retrieval when Alice stays on the same project. The daemon polls every 60 seconds; without this guard, it would run 60 retrievals/hour for no benefit.
