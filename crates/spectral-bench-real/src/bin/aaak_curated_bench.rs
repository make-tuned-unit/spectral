...
fn main() {
    ...
    let durable = [
        ("pref:diet", "I am vegetarian and do not eat any meat or fish"),
        ("pref:allergy", "I am severely allergic to shellfish and peanuts"),
        ("rule:nocall", "Never schedule anything for me before 9am, I do not take morning calls"),
        ("decision:stack", "I decided to standardize all my projects on Rust and Postgres"),
        ("fact:family", "My daughter Mia is five years old and starts school in September"),
        ("pref:comm", "I prefer concise written summaries over long meetings"),
    ];
    for (k, c) in durable {
    ...
    for i in 0..30 {
        brain
            .remember(&format!("chat{i}"), &format!("Grabbed a coffee and answered some emails around item {i} today"), Visibility::Private)
            .unwrap();
    }
    ...
    for (k, _) in durable {
        if let Ok(Some(m)) = brain.get_memory(&id_of(k)) {
            let pass = m.hall.as_deref().map(|h| ["fact","preference","decision","rule"].contains(&h)).unwrap_or(false) && m.signal_score >= 0.7;
            println!("  {k:<16} hall={:<12} signal={:.2}  {}", format!("{:?}", m.hall), m.signal_score, if pass { "✓ eligible" } else { "✗ excluded" });
        }
    }
    println!();

    let res = brain.aaak(AaakOpts::default()).unwrap();
    println!("budget: 170 tokens | AAAK used {} tokens, {} facts, wings: {:?}",
    ...
    let durable_in = durable.iter().filter(|(_, c)| res.formatted.contains(&c[..c.len().min(30)])).count();
    ...
    let recalled = brain
        ...
        .unwrap();
    let recall_has_diet = recalled.iter().any(|h| h.key == "pref:diet" || h.key == "pref:allergy");
    ...
    println!("\nverdict:");
    let ok = res.estimated_tokens <= 170 && durable_in >= 4 && chatter_in == 0 && aaak_has_diet && !recall_has_diet;
    println!("  AAAK is {}", if ok {
