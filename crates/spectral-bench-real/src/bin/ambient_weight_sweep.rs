...
fn main() {
    ...
    rw(
        &brain,
        "work:list",
        "The onboarding list tracks laptop, badge, and account setup",
        "work",
    );
    rw(
        &brain,
        "cooking:list",
        "The shopping list lists tomatoes, basil, and the list of spices",
        "cooking",
    );
    rw(
        &brain,
        "music:list",
        "The setlist list orders the opening songs for Friday",
        "music",
    );
    ...
    rw(
        &brain,
        "work:sched",
        "The release schedule schedules the deploy schedule for Thursday",
        "work",
    );
    rw(
        &brain,
        "cooking:sched",
        "The meal schedule plans dinners through Sunday",
        "cooking",
    );
    rw(
        &brain,
        "music:sched",
        "The practice schedule blocks an hour after work",
        "music",
    );
    ...
    rw(
        &brain,
        "work:budget",
        "The team budget covers two conference trips",
        "work",
    );
    rw(
        &brain,
        "cooking:budget",
        "The grocery budget caps the weekly shop at ninety",
        "cooking",
    );
    rw(
        &brain,
        "music:budget",
        "The gear budget budgets the budget for a new amp",
        "music",
    );
    ...
    let overrides: Vec<AmbCase> = vec![
        AmbCase {
            query: "sprint deploy checklist bugs",
            focus: "cooking",
            expect: "work:notes",
        },
        AmbCase {
            query: "garlic salt recipe",
            focus: "work",
            expect: "cooking:notes",
        },
        AmbCase {
            query: "tomatoes basil spices shopping",
            focus: "music",
            expect: "cooking:list",
        },
        AmbCase {
            query: "opening songs setlist friday",
            focus: "work",
            expect: "music:list",
        },
        AmbCase {
            query: "release deploy thursday",
            focus: "music",
            expect: "work:sched",
        },
        AmbCase {
    ...
    println!(
        "{:<22}{:>8}{:>8}{:>10}",
        "wing_match/mismatch", "A", "B", "verdict"
    );
    ...
    if let Some((wm, mp, a, b)) = best {
        ...
        println!(
            "shipped default (1.5/0.7): A={da}/{} B={db}/{}",
            amb.len(),
            overrides.len()
        );
    }
