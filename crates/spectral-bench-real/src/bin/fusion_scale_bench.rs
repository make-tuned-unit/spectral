...
fn main() {
    ...
    let subjects = ["The team", "Our group", "The committee", "The vendor", "The client", "My manager", "The squad", "Finance", "The board", "Engineering"];
    let verbs = ["reviewed", "updated", "flagged", "shipped", "postponed", "approved", "revised", "discussed", "audited", "finalized"];
    let objects = ["the quarterly report", "the budget review", "the status update", "the launch plan", "the hiring pipeline", "the roadmap", "the incident postmortem", "the vendor contract", "the design doc", "the migration"];
    let tails = ["ahead of the deadline", "after a long debate", "during the offsite", "before the release window", "with no objections", "over the weekend", "in the Monday sync", "against the forecast"];
    ...
    let answers: &[(&str, &str, &str, &str, &str)] = &[
        ...
        ("ans:university", "The state university confirmed its national research ranking rose again this year", "university", "collision", "The universe is vast and old"),
        ("ans:organization", "The new organization chart restructured every internal reporting line across teams", "organization", "collision", "The pipe organ boomed in the nave"),
        ("ans:policy", "The remote-work policy document was finalized after a long and contentious debate", "policy", "collision", "The police arrived within minutes"),
        ("ans:operative", "The operative clause in the settlement was struck by outside legal counsel", "operative", "collision", "The operation began at dawn"),
        ("ans:generalize", "The paper tried to generalize the finding beyond the narrow sampled population", "generalize", "collision", "The general addressed the troops"),
        ("ans:marketing", "Marketing rebuilt the entire landing page ahead of the delayed product launch", "marketing", "collision", "The open-air market was crowded today"),
        ("ans:relativity", "A guest seminar on the theory of relativity reshaped the physics curriculum", "relativity", "collision", "My relative called on Sunday night"),
        ("ans:doctors", "She finally consulted a doctor about the lingering cough last week", "doctors", "inflection", ""),
        ("ans:apartments", "We spent the whole weekend packing the apartment into cardboard boxes", "apartments", "inflection", ""),
        ("ans:engineers", "The startup keeps trying to recruit one more senior backend engineer", "engineers", "inflection", ""),
        ("ans:studies", "The longitudinal study tracked the same cohort for a full decade", "studies", "inflection", ""),
        ("ans:running", "He liked to run the coastal trail every morning before work", "running", "inflection", ""),
        ("ans:negotiations", "The contract negotiation stalled badly over the indemnity clause", "negotiations", "inflection", ""),
        ("ans:deploy", "The blue-green deploy cut the production rollout window to minutes", "blue-green deploy rollout", "neutral", ""),
        ("ans:sauna", "The lakeside cabin had a wood-fired sauna right by the dock", "lakeside cabin sauna", "neutral", ""),
        ("ans:tax", "The accountant flagged a missed quarterly estimated tax payment", "estimated tax payment", "neutral", ""),
        ("ans:violin", "She restrung the antique violin the night before the recital", "antique violin recital", "neutral", ""),
        ("ans:kayak", "We portaged the kayak around the second waterfall on the river", "portaged kayak waterfall", "neutral", ""),
        ("ans:compost", "The community garden finally started a shared compost program", "community garden compost", "neutral", ""),
        ("ans:telescope", "The rooftop telescope caught the lunar eclipse remarkably clearly", "rooftop telescope eclipse", "neutral", ""),
        ("ans:pension", "HR clarified the vesting schedule for the frozen pension plan", "pension vesting schedule", "neutral", ""),
        ("ans:allergy", "The clinic confirmed a seasonal pollen allergy after the panel", "seasonal pollen allergy", "neutral", ""),
        ("ans:mural", "A local artist painted a huge mural on the old depot wall", "artist mural depot", "neutral", ""),
    ];
    ...
    let cases: Vec<Case> = answers
        .iter()
        .enumerate()
        .map(|(i, (_, _, q, b, _))| Case { query: (*q).to_string(), answer_key: format!("a{i:02}"), bucket: b })
        .collect();
    ...
    let seed_corpus = |brain: &Brain| {
        ...
        for i in 0..300 {
            let c = format!(
                "{} {} {} {}",
                rng.pick(&subjects), rng.pick(&verbs), rng.pick(&objects), rng.pick(&tails)
            );
            brain.remember(&format!("distractor:{i}"), &c, Visibility::Private).unwrap();
        }
        ...
        // it back up.
        let suffixes = ["today", "this week", "again", "as usual", "reportedly", "overnight", "near dawn", "by all accounts"];
        for (ai, (_, _, _, _, collide)) in answers.iter().enumerate() {
            ...
            for (j, sfx) in suffixes.iter().enumerate() {
                ...
                brain.remember(&format!("coll:{ai}:{j}"), &c, Visibility::Private).unwrap();
            }
        }
        // Answer memories under opaque keys.
        for (i, (_, content, _, _, _)) in answers.iter().enumerate() {
            brain.remember(&format!("a{i:02}"), content, Visibility::Private).unwrap();
        }
    };

    eprintln!("ingesting porter brain (~330 memories)...");
    let porter = open(&std::env::temp_dir().join("spectral-fusion-scale-porter"), false);
    seed_corpus(&porter);
    eprintln!("ingesting fusion brain (~330 memories)...");
    let fusion = open(&std::env::temp_dir().join("spectral-fusion-scale-fusion"), true);
    seed_corpus(&fusion);
    ...
    let recall_at = |brain: &Brain, case: &Case, k: usize| -> bool {
        let hits = brain
            .recall_topk_fts(&case.query, &RecallTopKConfig { k: 40, ..Default::default() }, Visibility::Private)
            .unwrap();
    ...
    for &k in &ks {
        println!("-- recall@{k} --");
        println!("{:<12}{:>10}{:>10}{:>10}", "bucket", "porter", "fusion", "delta");
        for &bkt in &buckets {
            let subset: Vec<&Case> = cases.iter().filter(|c| bkt == "ALL" || c.bucket == bkt).collect();
    ...
    let bench_latency = |brain: &Brain| -> (f64, f64) {
        for c in &cases {
            let _ = brain.recall_topk_fts(&c.query, &RecallTopKConfig { k: 40, ..Default::default() }, Visibility::Private);
        }
        ...
        for _ in 0..20 {
            for c in &cases {
                ...
                let _ = brain
                    .recall_topk_fts(&c.query, &RecallTopKConfig { k: 40, ..Default::default() }, Visibility::Private)
                    .unwrap();
    ...
    println!("-- recall latency per query (ms), {} queries × 20 reps --", cases.len());
    println!("  porter  median={pm:.3}  mean={pa:.3}");
    println!("  fusion  median={fm:.3}  mean={fa:.3}");
    println!("  overhead: median {:+.1}%  mean {:+.1}%\n",
        (fm / pm - 1.0) * 100.0, (fa / pa - 1.0) * 100.0);
    ...
    for case in &cases {
        ...
        if rank != Some(0) {
            explained = true;
            let top3: Vec<String> = hits.iter().take(3).map(|h| {
                let c = &h.content;
                format!("{}={:?}", h.key, &c[..c.len().min(32)])
            }).collect();
            let r = rank.map(|i| (i + 1).to_string()).unwrap_or_else(|| "MISS".into());
            println!("  [{}] {:?} -> answer rank {r}; top3 {top3:?}", case.bucket, case.query);
        }
    }
    if !explained {
    ...
    for case in &cases {
        ...
        if p && !f {
            any = true;
            println!("  REGRESSED [{}] {:?} (answer {})", case.bucket, case.query, case.answer_key);
        }
    }
    if !any {
